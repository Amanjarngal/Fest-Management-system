...
import { Search, Calendar, ArrowUpDown, Shield, Store } from "lucide-react";
...
const UsersPage = () => {
  ...
  const getRoleBadge = (role) => {
    const base =
      "px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide flex items-center gap-1";
    switch (role) {
      case "admin":
        return (
          <span className={`${base} bg-red-600/90 text-white`}>
            <Shield size={12} /> Admin
          </span>
        );
      case "stallOwner":
        return (
          <span className={`${base} bg-blue-600/90 text-white`}>
            <Store size={12} /> Stall Owner
          </span>
        );
      case "organizer":
        return (
      ...
      default:
        return (
          <span className={`${base} bg-gray-700 text-white`}>User</span>
        );
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 mb-6">
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full bg-gray-900 border border-gray-700 rounded-full py-3 pl-12 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 shadow-md transition-all"
            />
            <Search
              size={20}
